...
class PipeField:
  ...
  def loop(self) -> list[Point]:
    ...
    for point in [northwards, eastwards, southwards, westwards]: 
      if self.connections.get(point, None) and self.starting_point in self.connections[point]: self.connections[self.starting_point].add(point)
  ...
  def inner_area(self) -> set[Point]:
    ...
    if inner_point not in loop: inner_area.add(inner_point)
  ...
  def expand_area(self, area: set[Point]) -> set[Point]:
    ...
    new_area = area.copy()
    for x,y in area:
      if (x+1,y) not in loop: new_area.add((x+1,y))
      if (x-1,y) not in loop: new_area.add((x-1,y))
      if (x,y-1) not in loop: new_area.add((x,y-1))
      if (x,y+1) not in loop: new_area.add((x,y+1))
